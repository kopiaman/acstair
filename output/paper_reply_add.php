...
if($inlineadd == ADD_INLINE)
	$templatefile = "paper_reply_inline_add.htm";
else
	$templatefile = "paper_reply_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: paper_reply_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing formalLetter - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_formalLetter = $pageObject->getControl("formalLetter", $id);
		$control_formalLetter->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing formalLetter - end
//	processing earlyBird - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_earlyBird = $pageObject->getControl("earlyBird", $id);
		$control_earlyBird->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing earlyBird - end
//	processing paymentMode - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_paymentMode = $pageObject->getControl("paymentMode", $id);
		$control_paymentMode->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing paymentMode - end
//	processing datePayment - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_datePayment = $pageObject->getControl("datePayment", $id);
		$control_datePayment->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing datePayment - end
//	processing dateSubmit - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_dateSubmit = $pageObject->getControl("dateSubmit", $id);
		$control_dateSubmit->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing dateSubmit - end
//	processing dateImportant - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_dateImportant = $pageObject->getControl("dateImportant", $id);
		$control_dateImportant->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing dateImportant - end


//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="paper")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["paperID"]==""){
			$avalues["paperID"] = prepare_for_db("paperID",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'paper_reply_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'paper_reply_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: paper_reply_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["replyID"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["replyID"]=postvalue("editid1");
	}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="paper")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["paperID"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
