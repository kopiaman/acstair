...
if($inlineadd == ADD_INLINE)
	$templatefile = "confee_inline_add.htm";
else
	$templatefile = "confee_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: confee_add.php');
...
//	processing fee - start
	...
	if($inlineAddOption)
	{
		...
		$control_fee->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing fee - end
//	processing quota - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_quota = $pageObject->getControl("quota", $id);
		$control_quota->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing quota - end
//	processing cat - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_cat = $pageObject->getControl("cat", $id);
		$control_cat->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing cat - end


//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="conference")
	{
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'confee_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'confee_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: confee_".$pageObject->getPageType().".php");
