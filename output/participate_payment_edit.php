...
$templatefile = ($inlineedit == EDIT_INLINE) ? "participate_payment_inline_edit.htm" : "participate_payment_edit.htm";
...
//	processing tx - begin
	...
	if($condition)
	{
		$control_tx = $pageObject->getControl("tx", $id);
		$control_tx->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing tx - end
//	processing currency - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_currency = $pageObject->getControl("currency", $id);
		$control_currency->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing currency - end
...
//	processing itemID - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_itemID = $pageObject->getControl("itemID", $id);
		$control_itemID->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing itemID - end
//	processing payDate - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_payDate = $pageObject->getControl("payDate", $id);
		$control_payDate->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing payDate - end
...
//	processing mode - end
	...
	//	do event
		...
		{		
			...
			//	processing tx - begin
				...
				if($condition)
				{
					$control_tx->afterSuccessfulSave();
				}
	//	processing tx - end
			//	processing currency - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_currency->afterSuccessfulSave();
				}
	//	processing currency - end
			//	processing amount - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
	...
	//	processing amount - end
			//	processing itemID - begin
				...
				if($condition)
				{
					$control_itemID->afterSuccessfulSave();
				}
	//	processing itemID - end
			//	processing payDate - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_payDate->afterSuccessfulSave();
				}
	//	processing payDate - end
			//	processing mode - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: participate_payment_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: participate_payment_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
{
	$data["tx"] = $evalues["tx"];
	$data["currency"] = $evalues["currency"];
	$data["amount"] = $evalues["amount"];
	$data["itemID"] = $evalues["itemID"];
	$data["payDate"] = $evalues["payDate"];
...
//	tx - 
	$value = $pageObject->showDBValue("tx", $data, $keylink);
	$showValues["tx"] = $value;
	$showFields[] = "tx";
		$showRawValues["tx"] = substr($data["tx"],0,100);

//	currency - 
	$value = $pageObject->showDBValue("currency", $data, $keylink);
	$showValues["currency"] = $value;
	$showFields[] = "currency";
		$showRawValues["currency"] = substr($data["currency"],0,100);
...
//	payStatus - 
	$value = $pageObject->showDBValue("payStatus", $data, $keylink);
	$showValues["payStatus"] = $value;
	$showFields[] = "payStatus";
		$showRawValues["payStatus"] = substr($data["payStatus"],0,100);

//	itemID - 
	$value = $pageObject->showDBValue("itemID", $data, $keylink);
	$showValues["itemID"] = $value;
	$showFields[] = "itemID";
		$showRawValues["itemID"] = substr($data["itemID"],0,100);

//	payDate - Short Date
	$value = $pageObject->showDBValue("payDate", $data, $keylink);
	$showValues["payDate"] = $value;
	$showFields[] = "payDate";
		$showRawValues["payDate"] = substr($data["payDate"],0,100);
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "participate_payment";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='participate_payment_view.php?".$viewlink."'\"");
