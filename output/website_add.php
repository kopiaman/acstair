...
$layout->blocks["top"][] = "details";$page_layouts["website_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "website_inline_add.htm";
else
	$templatefile = "website_add.htm";
...
//	processing cid - end
//	processing slider - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_slider = $pageObject->getControl("slider", $id);
		$control_slider->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'website_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'website_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: website_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["wid"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["wid"]=postvalue("editid1");
	}
...
if($readavalues)
{
	...
	$defvalues["slider"]=@$avalues["slider"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("slider_label","<label for=\"".GetInputElementId("slider", $id, PAGE_ADD)."\">","</label>");
