...
require_once("include/congallery_variables.php");
...
$page_layouts["congallery_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	while($data && $recordsCounter<$display_count) {
		...
		$row = array();
		$keylink = "";
		$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["pid"]));
	...
	//	cat - 
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("cat")))
				$class = ' rnr-field-number';
			$row["cat_class"] = $class;
		$rowinfo[] = $row;
		if ($b) {
...
$xt->load_template(GetTemplateName("congallery", "detailspreview"));
