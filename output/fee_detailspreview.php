...
$layout = new TLayout("detailspreview","Rounded1White_label1","MobileWhite_label1");
...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["fee_detailspreview"] = $layout;
...
{
	$where = "";
		$where .= GetFullFieldName("cid", $strTableName, false)."=".make_db_value("cid",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) {
	...
	//	period - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("period", $data, $keylink);
			$row["period_value"] = $value;
	//	fee - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("fee", $data, $keylink);
			$row["fee_value"] = $value;
	//	quota - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("quota", $data, $keylink);
			$row["quota_value"] = $value;
	//	cat - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("cat", $data, $keylink);
			$row["cat_value"] = $value;
		$rowinfo[] = $row;
		$data = $cipherer->DecryptFetchedArray($rs);
	}
...
$xt->load_template("fee_detailspreview.htm");
