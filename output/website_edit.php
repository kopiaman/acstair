...
$layout = new TLayout("edit2","Purific1Orange1","MobileOrange1");
...
$layout->blocks["top"][] = "details";$page_layouts["website_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "website_inline_edit.htm" : "website_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["wid"] = urldecode(postvalue("editid1"));
$savedKeys["wid"] = urldecode(postvalue("editid1"));
...
//	processing cid - begin
	...
	if($condition)
	{
		...
		$control_cid->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing cid - end
//	processing slider - begin
	$condition = 1;

	if($condition)
	{
		$control_slider = $pageObject->getControl("slider", $id);
		$control_slider->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing slider - end
	...
	//	processing cid - end
			//	processing slider - begin
				...
				if($condition)
				{
					$control_slider->afterSuccessfulSave();
				}
	//	processing slider - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["wid"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: website_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: website_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("slider"))
		$xt->assign("slider_fieldblock",true);
	else
		$xt->assign("slider_tabfieldblock",true);
	$xt->assign("slider_label",true);
	if(isEnableSection508())
		$xt->assign_section("slider_label","<label for=\"".GetInputElementId("slider", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("wid", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["wid"]));


//	wid - 
	$value = $pageObject->showDBValue("wid", $data, $keylink);
	$showValues["wid"] = $value;
	$showFields[] = "wid";
		$showRawValues["wid"] = substr($data["wid"],0,100);
...
//	slider - 
	$value = $pageObject->showDBValue("slider", $data, $keylink);
	$showValues["slider"] = $value;
	$showFields[] = "slider";
		$showRawValues["slider"] = substr($data["slider"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='website_view.php?".$viewlink."'\"");
