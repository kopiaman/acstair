...
include("include/user_fees_variables.php");
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "user_fees_inline_edit.htm" : "user_fees_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["bid"] = urldecode(postvalue("editid1"));
$savedKeys["bid"] = urldecode(postvalue("editid1"));
...
//	processing cid - end
//	processing cfid - begin
	$condition = 1;

	if($condition)
	{
		$control_cfid = $pageObject->getControl("cfid", $id);
		$control_cfid->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing cfid - end
	...
	//	processing cid - end
			//	processing cfid - begin
				...
				if($condition)
				{
					$control_cfid->afterSuccessfulSave();
				}
	//	processing cfid - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["bid"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: user_fees_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: user_fees_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("cfid"))
		$xt->assign("cfid_fieldblock",true);
	else
		$xt->assign("cfid_tabfieldblock",true);
	$xt->assign("cfid_label",true);
	if(isEnableSection508())
		$xt->assign_section("cfid_label","<label for=\"".GetInputElementId("cfid", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("bid", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["bid"]));


//	bid - 
	$value = $pageObject->showDBValue("bid", $data, $keylink);
	$showValues["bid"] = $value;
	$showFields[] = "bid";
		$showRawValues["bid"] = substr($data["bid"],0,100);
...
//	cfid - 
	$value = $pageObject->showDBValue("cfid", $data, $keylink);
	$showValues["cfid"] = $value;
	$showFields[] = "cfid";
		$showRawValues["cfid"] = substr($data["cfid"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='user_fees_view.php?".$viewlink."'\"");
