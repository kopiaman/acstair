...
$templatefile = ($inlineedit == EDIT_INLINE) ? "paper_reply_inline_edit.htm" : "paper_reply_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["replyID"] = urldecode(postvalue("editid1"));
$savedKeys["replyID"] = urldecode(postvalue("editid1"));
...
//	processing formalLetter - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_formalLetter = $pageObject->getControl("formalLetter", $id);
		$control_formalLetter->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing formalLetter - end
//	processing earlyBird - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_earlyBird = $pageObject->getControl("earlyBird", $id);
		$control_earlyBird->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing earlyBird - end
//	processing paymentMode - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_paymentMode = $pageObject->getControl("paymentMode", $id);
		$control_paymentMode->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing paymentMode - end
//	processing datePayment - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_datePayment = $pageObject->getControl("datePayment", $id);
		$control_datePayment->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing datePayment - end
//	processing dateSubmit - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_dateSubmit = $pageObject->getControl("dateSubmit", $id);
		$control_dateSubmit->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing dateSubmit - end
//	processing dateImportant - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_dateImportant = $pageObject->getControl("dateImportant", $id);
		$control_dateImportant->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing dateImportant - end
	...
	//	do event
		...
		{		
			...
			//	processing formalLetter - begin
				...
				if($condition)
				{
					$control_formalLetter->afterSuccessfulSave();
				}
	//	processing formalLetter - end
			//	processing earlyBird - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_earlyBird->afterSuccessfulSave();
				}
	//	processing earlyBird - end
			//	processing paymentMode - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_paymentMode->afterSuccessfulSave();
				}
	//	processing paymentMode - end
			//	processing datePayment - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_datePayment->afterSuccessfulSave();
				}
	//	processing datePayment - end
			//	processing dateSubmit - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_dateSubmit->afterSuccessfulSave();
				}
	//	processing dateSubmit - end
			//	processing dateImportant - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_dateImportant->afterSuccessfulSave();
				}
	//	processing dateImportant - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["replyID"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: paper_reply_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: paper_reply_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["formalLetter"] = $evalues["formalLetter"];
	$data["earlyBird"] = $evalues["earlyBird"];
	$data["paymentMode"] = $evalues["paymentMode"];
	$data["datePayment"] = $evalues["datePayment"];
	$data["dateSubmit"] = $evalues["dateSubmit"];
	$data["dateImportant"] = $evalues["dateImportant"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("replyID", $data)));
...
//	replyID - 
	$value = $pageObject->showDBValue("replyID", $data, $keylink);
	$showValues["replyID"] = $value;
	$showFields[] = "replyID";
		$showRawValues["replyID"] = substr($data["replyID"],0,100);
...
//	formalLetter - 
	$value = $pageObject->showDBValue("formalLetter", $data, $keylink);
	$showValues["formalLetter"] = $value;
	$showFields[] = "formalLetter";
		$showRawValues["formalLetter"] = substr($data["formalLetter"],0,100);

//	earlyBird - 
	$value = $pageObject->showDBValue("earlyBird", $data, $keylink);
	$showValues["earlyBird"] = $value;
	$showFields[] = "earlyBird";
		$showRawValues["earlyBird"] = substr($data["earlyBird"],0,100);

//	paymentMode - 
	$value = $pageObject->showDBValue("paymentMode", $data, $keylink);
	$showValues["paymentMode"] = $value;
	$showFields[] = "paymentMode";
		$showRawValues["paymentMode"] = substr($data["paymentMode"],0,100);

//	datePayment - 
	$value = $pageObject->showDBValue("datePayment", $data, $keylink);
	$showValues["datePayment"] = $value;
	$showFields[] = "datePayment";
		$showRawValues["datePayment"] = substr($data["datePayment"],0,100);

//	dateSubmit - 
	$value = $pageObject->showDBValue("dateSubmit", $data, $keylink);
	$showValues["dateSubmit"] = $value;
	$showFields[] = "dateSubmit";
		$showRawValues["dateSubmit"] = substr($data["dateSubmit"],0,100);

//	dateImportant - 
	$value = $pageObject->showDBValue("dateImportant", $data, $keylink);
	$showValues["dateImportant"] = $value;
	$showFields[] = "dateImportant";
		$showRawValues["dateImportant"] = substr($data["dateImportant"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "paper_reply";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='paper_reply_view.php?".$viewlink."'\"");
