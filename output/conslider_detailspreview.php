...
require_once("include/conslider_variables.php");
...
$page_layouts["conslider_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	//	headline - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("headline", $data, $keylink);
			$row["headline_value"] = $value;
			$format = $pSet->getViewFormat("headline");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("headline")))
				$class = ' rnr-field-number';
			$row["headline_class"] = $class;
		$rowinfo[] = $row;
		if ($b) {
			$rowinfo2[] = $row;
			$b = false;
...
$xt->load_template(GetTemplateName("conslider", "detailspreview"));
