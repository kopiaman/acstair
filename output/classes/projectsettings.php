...
function GetTableURL($table = "")
{
	...
	if("testmail" == $table)
		return "testmail";
	if("inviteLetter" == $table)
		return "inviteLetter";
	if("followup_payment" == $table)
		return "followup_payment";
	if("followup_abstract" == $table)
		return "followup_abstract";
	if("followup_user" == $table)
		return "followup_user";
	if("followup_payment_fullPaper" == $table)
		return "followup_payment_fullPaper";
	if("followup_full_paper" == $table)
		return "followup_full_paper";
	if("graph_presenter" == $table)
		return "graph_presenter";
	if("followup" == $table)
		return "followup";
	if("graph_participate" == $table)
		return "graph_participate";
	if("graph_user" == $table)
		return "graph_user";
...
function GetTableByShort($shortTName = "")
{
	...
	if("paper" == $shortTName)
		return "paper";
	if("conference" == $shortTName)
		return "conference";
	if("set_field" == $shortTName)
		return "set_field";
	if("set_cat" == $shortTName)
		return "set_cat";
	if("conslider" == $shortTName)
		return "conslider";
	if("congallery" == $shortTName)
		return "congallery";
	if("all" == $shortTName)
		return "all";
	if("conpartner" == $shortTName)
		return "conpartner";
	if("quote" == $shortTName)
		return "quote";
	if("company" == $shortTName)
		return "company";
	if("participate" == $shortTName)
		return "participate";
	if("set_fee" == $shortTName)
		return "set_fee";
	if("confees" == $shortTName)
		return "confees";
	if("payments" == $shortTName)
		return "payments";
	if("listener" == $shortTName)
		return "listener";
	if("certificate" == $shortTName)
		return "certificate";
	if("slot" == $shortTName)
		return "slot";
	if("webslider" == $shortTName)
		return "webslider";
	if("testmail" == $shortTName)
		return "testmail";
	if("inviteLetter" == $shortTName)
		return "inviteLetter";
	if("followup_payment" == $shortTName)
		return "followup_payment";
	if("followup_abstract" == $shortTName)
		return "followup_abstract";
	if("followup_user" == $shortTName)
		return "followup_user";
	if("followup_payment_fullPaper" == $shortTName)
		return "followup_payment_fullPaper";
	if("followup_full_paper" == $shortTName)
		return "followup_full_paper";
	if("graph_presenter" == $shortTName)
		return "graph_presenter";
	if("followup" == $shortTName)
		return "followup";
	if("graph_participate" == $shortTName)
		return "graph_participate";
	if("graph_user" == $shortTName)
		return "graph_user";
}
