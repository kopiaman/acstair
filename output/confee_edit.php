...
$templatefile = ($inlineedit == EDIT_INLINE) ? "confee_inline_edit.htm" : "confee_edit.htm";
...
//	processing fee - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_fee = $pageObject->getControl("fee", $id);
		$control_fee->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing fee - end
//	processing quota - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_quota = $pageObject->getControl("quota", $id);
		$control_quota->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing quota - end
//	processing cat - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_cat = $pageObject->getControl("cat", $id);
		$control_cat->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing cat - end
	...
	//	processing period - end
			//	processing fee - begin
				...
				if($condition)
				{
					$control_fee->afterSuccessfulSave();
				}
	//	processing fee - end
			//	processing quota - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_quota->afterSuccessfulSave();
				}
	//	processing quota - end
			//	processing cat - begin
							$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
				...
				if($condition)
				{
					$control_cat->afterSuccessfulSave();
				}
	//	processing cat - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: confee_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: confee_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["fee"] = $evalues["fee"];
	$data["quota"] = $evalues["quota"];
	$data["cat"] = $evalues["cat"];
}
...
//	fee - 
	$value = $pageObject->showDBValue("fee", $data, $keylink);
	$showValues["fee"] = $value;
	$showFields[] = "fee";
		$showRawValues["fee"] = substr($data["fee"],0,100);

//	quota - 
	$value = $pageObject->showDBValue("quota", $data, $keylink);
	$showValues["quota"] = $value;
	$showFields[] = "quota";
		$showRawValues["quota"] = substr($data["quota"],0,100);

//	cat - 
	$value = $pageObject->showDBValue("cat", $data, $keylink);
	$showValues["cat"] = $value;
	$showFields[] = "cat";
		$showRawValues["cat"] = substr($data["cat"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='confee_view.php?".$viewlink."'\"");
