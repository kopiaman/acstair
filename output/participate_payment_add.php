...
if($inlineadd == ADD_INLINE)
	$templatefile = "participate_payment_inline_add.htm";
else
	$templatefile = "participate_payment_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: participate_payment_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing itemID - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_itemID = $pageObject->getControl("itemID", $id);
		$control_itemID->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing itemID - end
//	processing currency - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_currency = $pageObject->getControl("currency", $id);
		$control_currency->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing tx - start
	...
	if($inlineAddOption)
	{
		...
		$control_tx->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing tx - end
//	processing payDate - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_payDate = $pageObject->getControl("payDate", $id);
		$control_payDate->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing payDate - end


//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="participate")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["userID"]==""){
			$avalues["userID"] = prepare_for_db("userID",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
		...
		if($avalues["cid"]==""){
			$avalues["cid"] = prepare_for_db("cid",$_SESSION[$sessionPrefix."_masterkey2"]);
		}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'participate_payment_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'participate_payment_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: participate_payment_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="participate")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["userID"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
	
	if(postvalue("masterkey2"))
		$_SESSION[$sessionPrefix."_masterkey2"] = postvalue("masterkey2");

	if(postvalue("mainMPageType")<>"add")
		$defvalues["cid"] = @$_SESSION[$sessionPrefix."_masterkey2"];	
