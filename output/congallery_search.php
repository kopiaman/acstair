...
include("include/congallery_variables.php");
...
$layout->blocks["top"][] = "search";$page_layouts["congallery_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "congallery_search.htm";
...
$params['shortTableName'] = 'congallery';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("photo"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("photo")] = GetTableURL($pageObject->pSet->getLookupTable("photo"));
	
	$pageObject->fillFieldToolTips("photo");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("photo");
	...
	if(isEnableSection508())
		$xt->assign_section("photo_label","<label for=\"".GetInputElementId("photo", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("photo_label", true);
	
	$xt->assign("photo_fieldblock", true);
	$xt->assignbyref("photo_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("photo_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("photo_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_photo", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("photo");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"photo", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"photo", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
