...
include("include/conpartner_variables.php");
...
$layout->blocks["bare"][] = "detailspreviewgrid";$page_layouts["conpartner_detailspreview"] = $layout;
...
if($rowcount) {
	...
	while($data && $recordsCounter<$display_count) {
		...
		$row = array();
		$keylink = "";
		$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["partid"]));
	...
	//	names - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("names", $data, $keylink);
			$row["names_value"] = $value;
		$rowinfo[] = $row;
		$data = $cipherer->DecryptFetchedArray($rs);
	}
...
$xt->load_template("conpartner_detailspreview.htm");
