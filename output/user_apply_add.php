...
add_nocache_headers();
include("include/user_apply_variables.php");
...
if($inlineadd == ADD_INLINE)
	$templatefile = "user_apply_inline_add.htm";
else
	$templatefile = "user_apply_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: user_apply_add.php');
...
//	processing cid - end
//	processing billCat - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_billCat = $pageObject->getControl("billCat", $id);
		$control_billCat->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing billCat - end
...
//	processing billCat - start
			...
			if($inlineAddOption)
			{
				$control_billCat->afterSuccessfulSave();
			}
//	processing billCat - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'user_apply_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'user_apply_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: user_apply_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["bid"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["bid"]=postvalue("editid1");
	}
...
//	billCat
	...
	if($display)
	{	
		$value = $pageObject->showDBValue("billCat", $data, $keylink);
		$showValues["billCat"] = $value;
		$showFields[] = "billCat";
	}	
		...
		$showRawValues["billCat"] = substr($data["billCat"],0,100);
		$showRawValues["amount"] = substr($data["amount"],0,100);
		$showRawValues["currency"] = substr($data["currency"],0,100);
		$showRawValues["billDate"] = substr($data["billDate"],0,100);
		$showRawValues["billStatus"] = substr($data["billStatus"],0,100);
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "user_apply";
		$options['firstTime'] = 1;
