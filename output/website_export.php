...
$layout->blocks["top"][] = "export";$page_layouts["website_export"] = $layout;
...
//	process selection
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["wid"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
...
$xt->display("website_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=website.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=website.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=website.xml");
	...
	{
		$values = array();
			$values["wid"] = $pageObject->showDBValue("wid", $row);
			$values["cid"] = $pageObject->showDBValue("cid", $row);
			$values["slider"] = $pageObject->showDBValue("slider", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=website.csv");
...
// write data rows
	...
	{
		$values = array();
			$values["wid"] = $pageObject->getViewControl("wid")->showDBValue($row, "");
			$values["cid"] = $pageObject->getViewControl("cid")->showDBValue($row, "");
			$values["slider"] = $pageObject->getViewControl("slider")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["wid"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["cid"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["slider"]).'"';
			echo $outstr;
		}
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Wid").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Cid").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Slider").'</td>';	
	}
	else
	{
		echo "<td>"."Wid"."</td>";
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["slider"]);
					else
						echo $values["slider"];
			echo '</td>';
			echo "</tr>";
